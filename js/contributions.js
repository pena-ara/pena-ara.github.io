...
function yearly(year) {
  ...
  catatan.sort((a, b) => { return b - a });
  document.querySelector('#catatan-activity').innerHTML = '';
  for (const month of ms) {
    const node = document.createElement('div');
    node.innerHTML = monthly(year, month, catatan);
    document.querySelector('#catatan-activity').appendChild(node);
  }

  graph(year, catatan, startDate, endDate);
...
function monthly(year, month, catatan) {
  const monthcatatan = catatan.filter(post => post.date.getMonth() === month);
  let liHtml = '';
  for (const post of monthcatatan) {
  ...
  <div class="contribution-activity-listing float-left col-12 col-lg-10">
    <div class="profile-timeline discussion-timeline width-full pb-4">
      <h3 class="profile-timeline-month-heading bg-white d-inline-block h6 pr-2 py-1">
        ${months[month]} <span class="text-gray">${monthcatatan.length > 0 ? monthcatatan[0].date.getFullYear() : year}</span>
      </h3>
      <div class="profile-rollup-wrapper py-4 pl-4 position-relative ml-3 js-details-container Details open">
        ...
        <button type="button" class="btn-link f4 muted-link no-underline lh-condensed width-full js-details-target "
          ...
          <span class="float-left ws-normal text-left">
            Created ${monthcatatan.length} post${monthcatatan.length > 1 ? 's' : ''}
          </span>
        ...
        <ul class="profile-rollup-content mt-1" data-repository-hovercards-enabled="" id="catatan-activity-ul">
        ${liHtml}
        </ul>
      </div>
    </div>
...
function graph(year, catatan, startDate, endDate) {
  const catatanStr = catatan.length === 1 ? "post" : "Post";
  if (year == now.getFullYear().toString()) {
    document.querySelector('#catatan-count').innerText = `${catatan.length}  ${catatanStr} in the last year`;
  } else {
    document.querySelector('#catatan-count').innerText = `${catatan.length}  ${catatanStr} in ${year}`;
  }

  let html = ``;
  const count = {};
  for (const post of catatan) {
